...
class SummarizationModel(object):
  ...
  def _add_encoder(self, encoder_inputs, seq_len):
    ...
    with tf.compat.v1.variable_scope('encoder'):
      ...
      encoder_outputs = tf.compat.v1.concat(axis=2, values=encoder_outputs) # concatenate the forwards and backwards states
  ...
  def _reduce_states(self, fw_st, bw_st):
    ...
    with tf.compat.v1.variable_scope('reduce_final_st'):
      ...
      # Apply linear layer
      old_c = tf.compat.v1.concat(axis=1, values=[fw_st.c, bw_st.c]) # Concatenation of fw and bw cell
      old_h = tf.compat.v1.concat(axis=1, values=[fw_st.h, bw_st.h]) # Concatenation of fw and bw state
  ...
  def _calc_final_dist(self, vocab_dists, attn_dists):
    ...
    with tf.compat.v1.variable_scope('final_distribution'):
      ...
      extra_zeros = tf.compat.v1.zeros((self._hps.batch_size, self._max_art_oovs))
      vocab_dists_extended = [tf.compat.v1.concat(axis=1, values=[dist, extra_zeros]) for dist in vocab_dists] # list length max_dec_steps of shape (batch_size, extended_vsize)

      # Project the values in the attention distributions onto the appropriate entries in the final distributions
      # This means that if a_i = 0.1 and the ith encoder word is w, and w has index 500 in the vocabulary, then we add 0.1 onto the 500th entry of the final distribution
      # This is done for each decoder timestep.
      # This is fiddly; we use tf.scatter_nd to do the projection
      batch_nums = tf.compat.v1.range(0, limit=self._hps.batch_size) # shape (batch_size)
      ...
      batch_nums = tf.compat.v1.tile(batch_nums, [1, attn_len]) # shape (batch_size, attn_len)
      indices = tf.compat.v1.stack( (batch_nums, self._enc_batch_extend_vocab), axis=2) # shape (batch_size, enc_t, 2)
      shape = [self._hps.batch_size, extended_vsize]
      attn_dists_projected = [tf.compat.v1.scatter_nd(indices, copy_dist, shape) for copy_dist in attn_dists] # list length max_dec_steps (batch_size, extended_vsize)
  ...
  def _add_seq2seq(self):
    ...
    with tf.compat.v1.variable_scope('seq2seq'):
      ...
      with tf.compat.v1.variable_scope('embedding'):
        ...
        emb_dec_inputs = [tf.compat.v1.nn.embedding_lookup(embedding, x) for x in tf.compat.v1.unstack(self._dec_batch, axis=1)] # list length max_dec_steps containing shape (batch_size, emb_size)
      ...
      with tf.compat.v1.variable_scope('output_projection'):
        ...
        vocab_dists = [tf.compat.v1.nn.softmax(s) for s in vocab_scores] # The vocabulary distributions. List length max_dec_steps of (batch_size, vsize) arrays. The words are in the order they appear in the vocabulary file.
      ...
      if hps.mode in ['train', 'eval']:
        ...
        with tf.compat.v1.variable_scope('loss'):
          if FLAGS.pointer_gen:
            ...
            batch_nums = tf.compat.v1.range(0, limit=hps.batch_size) # shape (batch_size)
            for dec_step, dist in enumerate(final_dists):
              targets = self._target_batch[:,dec_step] # The indices of the target words. shape (batch_size)
              indices = tf.compat.v1.stack( (batch_nums, targets), axis=1) # shape (batch_size, 2)
            ...
            self._loss = tf.contrib.seq2seq.sequence_loss(tf.compat.v1.stack(vocab_scores, axis=1), self._target_batch, self._dec_padding_mask) # this applies softmax internally
  ...
  def _add_train_op(self):
    ...
    with tf.compat.v1.device("/gpu:0"):
      grads, global_norm = tf.compat.v1.clip_by_global_norm(gradients, self._hps.max_grad_norm)
